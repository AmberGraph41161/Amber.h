...
namespace AMBER
{
    ...
    void presskey(char key, double delay, bool log)
    {
        ...
        case ')':
            ...
            keybd_event(VK_SHIFT, 0, KEYEVENTF_KEYUP, 0);
            break;
            //RANDOM LOWERCASE SPECIAL CHARS
        case '`':
            break;

            //RANDOM UPPERCASE SPECIAL CHARS
        case '~':
            break;
        }
        ...
        {
            keybd_event(std::toupper(key), 0, KEYEVENTF_EXTENDEDKEY, 0);
            keybd_event(std::toupper(key), 0, KEYEVENTF_KEYUP, 0);
        }
